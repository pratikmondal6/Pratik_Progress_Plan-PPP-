...
function bookingDateValue(request){return String(request.date||request.startDate||request.start_date||request.dateTime||request.start||"").slice(0,10)}
function bookingTimeValue(request){
  const raw=String(request.time||request.startTime||request.start_time||"").trim(),match=raw.match(/(?:^|T)([01]?\d|2[0-3]):([0-5]\d)/);return match?`${String(Number(match[1])).padStart(2,"0")}:${match[2]}`:"";
}
...
function renderBookingRequests(){
  ...
  host.innerHTML=`<div class="booking-table-wrap"><table class="booking-table"><thead><tr><th>Request</th><th>Person</th><th>Date</th><th>Time</th><th>Duration</th><th>Actions</th></tr></thead><tbody>${pending.map(request=>`<tr><td><strong>${escapeHtml(bookingTitle(request))}</strong>${request.note?`<div class="booking-note">${escapeHtml(request.note)}</div>`:""}</td><td>${escapeHtml(request.name||"Unknown")}</td><td>${escapeHtml(bookingDateLabel(bookingDateValue(request)))}</td><td>${escapeHtml(bookingTimeLabel(bookingTimeValue(request)))}</td><td>${clamp(Number(request.duration)||30,15,240)} min</td><td><div class="booking-row-actions"><button class="btn green" data-booking-action="accept" data-booking-id="${escapeHtml(request.id)}">Accept</button><button class="btn danger" data-booking-action="decline" data-booking-id="${escapeHtml(request.id)}">Decline</button></div></td></tr>`).join("")}</tbody></table></div>`;
}
...
function refreshBookingRequests(silent=false){if(!silent)document.getElementById("bookingRequestList").innerHTML='<div class="booking-empty">Loading requests…</div>';bookingJsonp("list",{},data=>{if(data)sharedBookingRequests=Array.isArray(data.requests)?data.requests:[];if(data)sharedBookingRequests.filter(request=>String(request.status).toLowerCase()==="accepted").forEach(storeAcceptedBooking);renderBookingDashboard();if(data&&!silent)showToast("Booking dashboard refreshed")})}
