function renderSettings(){
  ...
  document.getElementById("goalTargetSettings").innerHTML=GOALS.map(goal=>`<div class="setting-row"><label>${goal.icon} ${goal.label}</label><input type="number" min="5" max="480" step="5" value="${goalTarget(goal.key)}" data-goal-target="${goal.key}"></div>`).join("");
